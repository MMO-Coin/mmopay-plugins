...
class vB_PaidSubscriptionMethod_mmocoin_pay extends vB_PaidSubscriptionMethod
{
	...
	* refused outright rather than falling back to a weaker check, because a
	* weaker fallback is exactly what let an unauthenticated request approve a
	* subscription for free in the previous version of this file.
	*
	...
	*/
	function verify_payment()
	{
		...
		// a second API round-trip here would only be one more thing that can
		...
		// Verify Event Type & Dispatch Approval. Note: an EMPTY event is no
		// longer treated as approval -- a signed payload always names its
		// event explicitly, so a blank one has nothing legitimate behind it.
...
/*======================================================================*\
|| ####################################################################
|| # MMOCoin Pay Gateway Engine - v1.2.0 (USD pricing + security-hardened)
|| ####################################################################
\*======================================================================*/
?>
